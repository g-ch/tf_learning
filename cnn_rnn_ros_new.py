...
model_path = "/home/ubuntu/chg_workspace/3dcnn_yaw_in_map/model/cnn_rnn/01/simulation_cnn_rnn100.ckpt"
    ...
    with tf.Session(config=config) as sess:
        ...
        while not rospy.is_shutdown():
            if new_msg_received:
                ...
                data2_vl = np.ones([1, states_compose_num[0]]) * velocity_odom_linear
                data2_va = np.ones([1, states_compose_num[1]]) * velocity_odom_angular
                data2_to_feed = np.concatenate([data2_vl, data2_va], axis=1)
