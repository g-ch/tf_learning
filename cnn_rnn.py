...
def myrnn(x, input_len, output_len, raw_batch_size, time_step, state_len):
    ...
    """
    w = tf.get_variable("weight_x", [input_len, state_len], initializer=tf.truncated_normal_initializer(stddev=0.1))
    u = tf.get_variable("weight_s", [state_len, state_len], initializer=tf.truncated_normal_initializer(stddev=0.1))
    v = tf.get_variable("weight_y", [state_len, output_len], initializer=tf.truncated_normal_initializer(stddev=0.1))
...
def generate_shuffled_array(start, stop):
    """
...
if __name__ == '__main__':
    ...
    data1, data2, label = generate_sin_x_plus_y(data_num, cube_dim, concat_paras["dim2"], rnn_paras["output_len"], 0.1, 0, 1, 2)
    ...
    ''' Optimizer '''
    label_batch = np.zeros((rnn_paras["raw_batch_size"], rnn_paras["output_len"]))  # Temprate use
    loss = tf.reduce_mean(tf.square(label_batch - result))
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(epoch_num):
            ...
            sequence = generate_shuffled_array(rnn_paras["time_step"] - 1, total_data_num)
            # start batches
            ...
            print('loss=%s' % sess.run(loss, feed_dict={cube_data: data1_batch, line_data: data2_batch, reference: label_batch}))
