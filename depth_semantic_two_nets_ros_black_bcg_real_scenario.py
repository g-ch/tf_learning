...
model_path = "/home/gg/intel_nav_ws/src/EAI-E1/scripts/model/dep_semantic/simulation_cnn_rnn400.ckpt"
...
def callBackSemantic(objects):
	...
	for box in objects.bounding_boxes:
		...
		semantic_origin_size[range_y_min - 1:range_y_max, range_x_min - 1:range_x_max] = (np.trunc(label * 32)).astype(np.uint8)

		# print ( semantic_origin_size[range_y_min - 1:range_y_max, range_x_min - 1:range_x_max])
		...
		# print ("jjjjjjj")
		# cv2.imshow("semantic", semantic_image[0,:,:,:])
		# cv2.waitKey(5)
...
def callBackDepth(img):
	...
	depth_image_list = depth_image_list * 0.001

	# lantern: limits on distance
	depth_image_list[depth_image_list > 4.5] = 0.0
	depth_image_list[np.isnan(depth_image_list)] = 0.0
	depth_image_list = depth_image_list * 56

	# unsigned int format
	depth_image_trunc = np.trunc(depth_image_list).astype(np.uint8)

	# reshape to the size to feed to nn
	depth_image = np.array(depth_image_trunc).reshape([1, img_height, img_wid, img_channel])
...
if __name__ == '__main__':
	...
	rospy.Subscriber("/kinect2/sd/image_depth_rect", Image, callBackDepth)
